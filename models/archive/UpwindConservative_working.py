...
calculating bed load only

Seems like my bed is evolving too slow.
        ...
        while t < t_max:
            ...
            for i in range(4,number_nodes-3):
                ...
                if sed_flag==True:
                    ...
                    Qbed[i]=0
                    if Qb_star[i] != 0 and abs(U[i]) > 0.001:
                         Qbed[i] = Qbu_tmp[i]/U[i]
                    L = 6 * h[i]
                    Qbu[i] = Qbu_tmp[i] - (dt/dx)*dQbdx[i] + (dt/L)*(Qb_star[i] - Qbed[i])

                    ''' Calculate ub '''
                    Qbed[i]=0
                    if Qb_star[i] != 0 and abs(U[i]) > 0.001:
                         Qbed[i] = Qbu[i]/U[i]
                    Zbed[i] = Zbed_tmp[i] - (dt/dx)*(1./(1-pm))*(1./L)*(Qb_star[i] - Qbed[i])
